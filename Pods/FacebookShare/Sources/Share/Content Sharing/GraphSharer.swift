...
public final class GraphSharer<Content: ContentProtocol> {
   ...
   */
    public init(content: Content, message :String) {
        sdkSharer = FBSDKShareAPI()
        sdkShareDelegate = SDKSharingDelegateBridge()
        sdkSharer.message = message
...
extension GraphSharer {
   ...
   */
  @discardableResult
    public static func share(_ content: Content, message : String ,completion: ((ContentSharerResult<Content>) -> Void)? = nil) throws -> GraphSharer {
    let sharer = self.init(content: content, message : message)
